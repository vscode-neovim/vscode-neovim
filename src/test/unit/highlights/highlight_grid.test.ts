            ...
            events: [
                {
                    ...
                    // TODO: This functionality differs for the waving emoji, for some reason
                    lineText: "你 yo",
                    tabSize: 4,
                },
            ],
...
describe("isDouble", () => {
    [
        ...
        it(`${expectedDouble ? "should" : "should not"} consider '${char}' to be double-width`, () => {
            assert.equal(HighlightGrid.isDouble(char), expectedDouble);
        });
    });
});
...
describe("getWidth", () => {
    [
        ...
        it(testName, () => {
            assert.equal(HighlightGrid.getWidth(text, 4), expectedWidth);
        });
    });
});
