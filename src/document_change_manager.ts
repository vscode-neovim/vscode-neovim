...
export class DocumentChangeManager implements Disposable {
    ...
    private onChangeTextDocumentLocked = async (e: TextDocumentChangeEvent, origText: string): Promise<void> => {
        if (config.autoGeneratedDocumentSchemes.includes(e.document.uri.scheme)) {
            return this.onGeneratedDocTextChange(e);
        }
    ...
    // For auto-generated documents, we directly replace the content.
    // These documents frequently change rapidly and in large volumes, which can
    // lead to synchronization issues and errors such as cursor misalignment.
    // Additionally, we do not process dotrepeat or cursor updates, as these are unnecessary.
    private async onGeneratedDocTextChange({ document }: TextDocumentChangeEvent) {
        logger.log(document.uri, LogLevel.Debug, `Handle generated document change for ${document.uri}`);
        const bufId = this.main.bufferManager.getBufferIdForTextDocument(document);
        if (!bufId) {
            logger.log(document.uri, LogLevel.Warning, `No neovim buffer for ${document.uri}`);
            return;
        }
        const text = document.getText();
        const lines = text.split(document.eol === EndOfLine.LF ? "\n" : "\r\n");
        const bufTick = await actions.lua("handle_generated_document_changes", bufId, lines);
        this.bufferSkipTicks.set(bufId, bufTick + 1);
    }
}
