...
support nvim bindings. Simpler pieces of code include the shortcuts in `package.json` and the vimscript files, there are
always ways to better support nvim bindings.
...
-   `src/`: The main extension code. This is where most of the work happens.
    ...
    -   `main_controller.ts`: The main controller for the extension. This is where nvim is spawned, communication
    -   `extension.ts`: The entrypoint for the extension.
    -   All other files are managers responsible for syncing a specific aspect of the editor with nvim, such as mode,
        between vscode and nvim is set up, and all managers are initialized.
...
-   Listen to custom events from nvim through `handleNotification`.
...
-   Provides `this.main.modeManager.isInsertMode`, `this.main.modeManager.isInsertMode` and equivalent properties for
    other modes.
-   Provides a `onModeChange` subscription that can be used to listen to mode changes.
